...
dataset = pd.read_excel('cleanedBTCValues.xls') #FINALLSTMDatasetBitcoin.xlsx
nrows = 2398
values = dataset.iloc[:,3].values #Getting vwap scores [2:5]
# Changed 
...
# print("---------------------------------")
# print("Reframed Values: ",reframed.head())
# print("---------------------------------")
...
d= dataset.iloc[n_train_days+1:nrows]['timestamp']
...
# Create a csv
actual= pd.DataFrame(inv_y, columns= ['Actual Value'])
predicted=pd.DataFrame(inv_yhat, columns= ['Predicted Value'])
results=pd.concat([predicted,actual], axis=1 )
